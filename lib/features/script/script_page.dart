...
class _ScriptPageState extends ConsumerState<ScriptPage> {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Row(
          ...
          children: [
            Icon(Icons.code_rounded, color: kAccentColor),
            SizedBox(width: 8),
            Text('脚本库'),
          ],
        ),
      ...
      body: Column(
        children: [
          ModernHeroCard(
            title: 'Lua 脚本工作台',
            subtitle: '集中管理内置脚本、用户脚本和运行日志，适合快速测试与复用。',
          ...
          if (_isRunning)
            const Padding(
              ...
              child: LoadingCard(message: '脚本运行中...'),
            ),
          Expanded(
            child: scripts.isEmpty
                ? const EmptyIllustration(
                    ...
                    title: '暂无脚本',
                    subtitle: '点击右上角 + 创建新脚本，或从悬浮窗同步脚本。',
                  )
                : ListView.builder(
  ...
  Widget _buildScriptCard(ScriptModel script) {
    final icon = script.isBuiltin
        ? Icons.inventory_2_rounded
        : script.isAiGenerated
            ? Icons.smart_toy_rounded
            : Icons.person_rounded;
    final typeLabel = script.isBuiltin
        ? '内置'
        : script.isAiGenerated
            ? 'AI'
            : '用户';
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              StatusPill(icon: Icons.sell_rounded, label: typeLabel),
              ...script.tags.map(
                (tag) => StatusPill(icon: Icons.tag_rounded, label: tag),
              ),
              if (script.executionCount > 0)
                StatusPill(
                  icon: Icons.bolt_rounded,
                  label: '已执行 ${script.executionCount} 次',
                  color: kSuccessColor,
                ),
            ],
          ),
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.receipt_long_rounded, color: kAccentColor),
            SizedBox(width: 8),
            Text('脚本运行日志'),
          ],
        ),
      ...
      body: logs.isEmpty
          ? const EmptyIllustration(
              icon: Icons.receipt_long_rounded,
              title: '暂无运行日志',
              subtitle: '运行脚本后会自动保存日志，便于回看输出和错误信息。',
            )
          : ListView.builder(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: kSplashColor.withValues(alpha: 0.38),
              borderRadius: BorderRadius.circular(12),
            ),
            child: Text(
              output.length > 150 ? '${output.substring(0, 150)}...' : output,
              style: const TextStyle(
                fontFamily: 'monospace',
                fontSize: 11,
                color: kAccentColor,
