...
class _SettingsPageState extends ConsumerState<SettingsPage> {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.tune_rounded, color: kAccentColor),
            SizedBox(width: 8),
            Text('设置'),
          ],
        ),
      ...
      body: ListView(
        ...
        children: [
          const ModernHeroCard(
            title: '应用配置中心',
            subtitle: '集中管理 AI 接口、悬浮窗、读取深度、Root 状态和项目信息。',
          ...
          ModernCard(
            ...
            child: const ListTile(
              leading: Icon(Icons.code_rounded, color: kAccentColor),
              title: Text('GitHub 开源地址'),
              subtitle: Text(
                'https://github.com/yl985211/gg-ai-modifier',
                style: TextStyle(fontSize: 12, color: kAccentColor),
              ),
              trailing: Icon(Icons.open_in_new_rounded, color: kTextSecondary),
            ),
          ),
          const SizedBox(height: 12),
          const ModernCard(
            ...
            child: Row(
              children: [
                Icon(Icons.warning_amber_rounded, color: kWarningColor, size: 20),
                SizedBox(width: 8),
                Expanded(
                  child: Text(
                    '本工具仅供学习研究和个人单机游戏使用，禁止用于在线竞技游戏。',
                    style: TextStyle(fontSize: 12, color: kWarningColor, height: 1.35),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildRootStatusCard() {
    final granted = _rootStatus.contains('✅');
    return ModernCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              StatusPill(
                icon: granted ? Icons.check_circle_rounded : Icons.security_rounded,
                label: granted ? '已授权' : '待检测',
                color: granted ? kSuccessColor : kAccentColor,
              ),
          ...
          const Text(
            '需要 Root 权限才能读写其他进程内存。点击下方按钮会触发 Magisk 授权弹窗。',
    ...
    return ModernCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              StatusPill(
                ...
                label: _overlayEnabled ? '已开启' : '未开启',
                color: _overlayEnabled ? kSuccessColor : kAccentColor,
              ),
              const SizedBox(width: 10),
              const Expanded(
                child: Text(
              ...
              Switch(value: _overlayEnabled, onChanged: _toggleOverlay),
            ],
          ),
          const SizedBox(height: 10),
          const Text(
          ...
          Row(
            children: [
              const Icon(Icons.autorenew_rounded, size: 17, color: kTextSecondary),
              ...
              Switch(value: _autoStartOverlay, onChanged: _toggleAutoStart),
            ],
          ),
        ],
      ),
    ...
    return ModernCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          const Text(
            '限制发送给 AI 的搜索结果数量，节省 Token 并防止上下文溢出。当结果过多时，仅发送样本和统计数据。',
            style: TextStyle(fontSize: 12.5, color: kTextSecondary, height: 1.4),
          ),
          const SizedBox(height: 12),
          Wrap(
